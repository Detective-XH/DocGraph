...
func TestRunMigrations_FreshDB(t *testing.T) {
	...
	// 10 rows in schema_migrations (001–010).
	...
	for _, tbl := range []string{"nodes", "edges", "files", "unresolved_refs", "project_metadata", "file_history", "embeddings", "section_chunks", "document_metadata", "governance_metadata", "research_metadata", "domain_packs", "domain_pack_fields", "nodes_fts", "section_chunks_fts", "schema_migrations", "entities", "entity_mentions"} {
...
func TestRunMigrations_OldDBBaseline(t *testing.T) {
	...
	// 10 rows inserted (legacy 001–003 are baselined; 004–010 run normally).
	if n := countMigrationRows(db); n != 10 {
		t.Errorf("expected 10 migration rows, got %d", n)
	}
	if v := getUserVersion(db); v != 10 {
		t.Errorf("expected user_version=10 after applying post-baseline migrations, got %d", v)
	}
	for _, tbl := range []string{"section_chunks", "section_chunks_fts", "document_metadata", "governance_metadata", "research_metadata", "domain_packs", "domain_pack_fields", "entities", "entity_mentions"} {
		if !tableExists(db, tbl) {
...
func TestRunMigrations_IdempotentReopen(t *testing.T) {
	...
	// Still exactly 10 rows — no duplicates.
	if n := countMigrationRows(db); n != 10 {
...
func TestRunMigrations_ChecksumMismatch(t *testing.T) {
	...
	// DB should still have 10 rows (unchanged).
	if n := countMigrationRows(db); n != 10 {
		t.Errorf("expected 10 migration rows after mismatch, got %d", n)
	}
}
...
func TestRunMigrations_WorkspaceMixedState(t *testing.T) {
	...
	if n := countMigrationRows(normalDB); n != 10 {
		t.Errorf("normal DB: expected 10 migration rows, got %d", n)
	}
...
func TestMigration005To007_Idempotent(t *testing.T) {
	...
	// First open: applies all 10 migrations.
	...
	if n := countMigrationRows(db1); n != 10 {
		t.Errorf("after first open: expected 10 migration rows, got %d", n)
	}
	db1.Close()
	...
	if n := countMigrationRows(db2); n != 10 {
		t.Errorf("after second open: expected 10 migration rows (no re-runs), got %d", n)
	}
